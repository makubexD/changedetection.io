...
**Request** and read the **Fetch Method** radios. Three are listed — `Basic fast
Plaintext/HTTP Client`, a browser one, and `System settings default` — and the
browser one must say
...
| `cannot listen on the TCP port: ... address already in use` (exit 126) | `podman pod ps` | A pod from an earlier `-WithBrowser` run still owns the port — its infra container keeps the binding even after the app container is gone. Current `run.ps1` clears both topologies on every start; if you meet this from any other path: `podman pod rm -f changedetection-pod`, or pick another port with `-Port`. |

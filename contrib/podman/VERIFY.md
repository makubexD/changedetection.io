...
| runtime | the fork's `contrib/runtime` patches reached the container and behave — nothing in the UI shows this, so it is asserted here or not at all. **Four lines, one pair per patch**: that the module loads and answers correctly, then whether it is confirmed against the live application — the [decimals filter](DEPLOY.md#the-forks-runtime-patches) on `flask_app`, and [conditional requests](WATCHING.md#2-conditional-requests-this-fork-sends-them) on the plain fetcher. Only the second of each pair proves a patch reached the *running application*; a `WARN` there leaves that one thing unproven and fails nothing |
